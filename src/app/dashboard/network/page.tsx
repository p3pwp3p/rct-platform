...
const NAV = [
  { label: '글로벌 개요', href: '/dashboard/network', active: true },
  ...
  return (
    <>
      <style>{`
        ...
        .network-nav-item:hover { background: rgba(255,255,255,0.04); }
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', height: '100%', overflow: 'hidden' }}>
        ...
        <aside style={{ background: 'var(--bg-surface)', borderRight: '1px solid var(--border-primary)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div className="network-sidebar-header">Command Center</div>
          <nav style={{ flex: 1, overflowY: 'auto' }}>
            {NAV.map(item => (
              <Link key={item.label} href={item.href} className={`network-nav-item${item.active ? ' active' : ''}`}>
                {item.active && (
            ...
            <div style={{ borderTop: '1px solid var(--border-primary)', marginTop: 8 }}>
              <div className="network-sidebar-header" style={{ borderBottom: 'none' }}>Infrastructure</div>
              <Link href="/dashboard" className="network-nav-item">보안 프로토콜</Link>
            </div>
          </nav>
        </aside>
