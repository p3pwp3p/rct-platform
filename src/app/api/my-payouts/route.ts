...
export async function GET(req: NextRequest) {
  try {
    ...
    // ── 0. 공개 가능한 보고서만 추림 ────────────────────────────────────────
    // 수당은 계산·저장되어도 관리자가 컨펌(status='confirmed')하기 전에는 회원에게
    // 보이면 안 된다. 검토 중 금액이 바뀔 수 있어서, 확정된 것만 노출한다.
    // 'paid' 는 이미 송금까지 끝난 것이라 당연히 공개.
    const VISIBLE = ['confirmed', 'paid']
    const { data: allReportRows, error: repErr } = await admin
      .from('profit_reports')
      .select('id, date_from, date_to, status')
    if (repErr) throw repErr

    const visibleIds = new Set(
      (allReportRows ?? []).filter(r => VISIBLE.includes(r.status)).map(r => r.id),
    )
    // 아직 컨펌 전인 보고서가 있으면 "정산 준비중"으로 알려주기 위한 수
    const preparingCount = (allReportRows ?? []).filter(r => !VISIBLE.includes(r.status)).length
    ...
    if (!allRows.length) return NextResponse.json({ rows: [], totals, breakdown, monthly: [], breakdownByMonth: {}, rowCount: 0, preparingCount })

    // 3. 표시는 최근 500건으로 제한
    const rows = allRows.slice(0, 500)

    // 4. 연결된 profit_reports 정보 join — 위(0단계)에서 이미 받아둔 걸 재사용
    const reportMap = new Map<string, { date_from: string; date_to: string; status: string }>(
      (allReportRows ?? []).map(r => [r.id, { date_from: r.date_from, date_to: r.date_to, status: r.status }])
    )
    ...
    return NextResponse.json({ rows: enriched, totals, breakdown, monthly, breakdownByMonth, rowCount: allRows.length, preparingCount })
  } catch (e: unknown) {
